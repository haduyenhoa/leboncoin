...
- (NSString *) ocrImage: (UIImage *) uiImage
{
    ...
    char* text = _tesseract->TesseractRect(imageData,(int)bytes_per_pixel,(int)bytes_per_line, 0, 0,(int) imageSize.height,(int) imageSize.width);
...
- (NSString*)analyseImage:(UIImage *)image
{
    @synchronized(self)
    {
        ...
        _tesseract->SetImage((const unsigned char *) pixels, width, height, sizeof(uint32_t), baseWidth);
        
        _tesseract->Recognize(NULL);
        const char *utf8Text = _tesseract->GetUTF8Text();
